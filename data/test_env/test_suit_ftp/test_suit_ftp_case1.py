...
class test_suit_ftp_case1(TestCaseBase):
    def test_case_main(self, case_results):
        ...
        upload_path = SC.PRIVATE_FTP_FTP_SERVER_ULPATH
        ul_file = SC.PRIVATE_FTP_FTP_UPLOAD_FILE
        time_expire = int(SC.PRIVATE_FTP_FTP_UPLOAD_EXPIRETIME)
    ...
    #        if not search_text("124.127.126.216"):
            if search_text("59.61"):
                ...
                entertext_edittext_by_index(5,upload_path)
                goback()
            ...
            if wait_for_fun(lambda:search_text("100KB.bin"), True, 100):
                ...
                click_textview_by_text("Device file browser")
                sleep(3)
                
                for i in range(0, repeat_times):
                    total_times = total_times+1
                    click_textview_by_text(ul_file)
                    click_textview_by_text("Upload")
                    click_textview_by_text("Ok")
                    ...
                    while(1):
                        ...
                        if t_expire > time_expire:
                            ...
                            flag1 = False
                            break
                        if (search_text("Upload completed")):
                            click_textview_by_text("Ok")
                            time_end=time.strftime('%X',time.localtime(time.time()))
                            flag1=True
                            break
                        if (search_text("Upload failed")):
                            click_textview_by_text("Ok")
                            flag1=False
                            break
                        if (search_text("Upload cancelled")):
                            click_textview_by_text("Ok")
                            flag1=False
                            break
                    if  flag1==True:
                        ...
                        if string.find(ul_file, "KB") != -1:
                            sizestr=ul_file.split("KB")
                            speed=float(sizestr[0])/float(sec)
                            qsst_log_msg("Times:"+str(i+1)+'\t'+str(speed)+"KB/s")
    
                        elif string.find(ul_file, "MB") !=-1:
                            sizestr=ul_file.split("MB")
