...
class Camera(Base):
    ...
    def take_picture(self):
        ...
        if(osInfo == 'Linux-Phone'):
            beforePictures = os.listdir(os.sep+'storage'+os.sep+'sdcard0'+os.sep+'DCIM'+os.sep+'Camera'+os.sep)
        click_imageview_by_id('shutter_button')
        sleep(4)

        if(osInfo == 'Linux-Phone'):
            afterPictures = os.listdir(os.sep+'storage'+os.sep+'sdcard0'+os.sep+'DCIM'+os.sep+'Camera'+os.sep)
    ...
    def take_video(self,second):
        ...
        osInfo = get_platform_info()
        if(osInfo == 'Linux-Phone'):
            beforeFiles = os.listdir(os.sep+'storage'+os.sep+'sdcard0'+os.sep+'DCIM'+os.sep+'Camera'+os.sep)
        click_imageview_by_id('shutter_button')
        sleep(second)
        #check whether the vedio is recoding
        local_assert(True,search_view_by_id('recording_time'))
        if not can_continue():
            return False
        ...
        sleep(4)
        if(osInfo == 'Linux-Phone'):
            afterFiles = os.listdir(os.sep+'storage'+os.sep+'sdcard0'+os.sep+'DCIM'+os.sep+'Camera'+os.sep)
