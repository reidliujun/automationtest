...
class test_suit_combination_case5(TestCaseBase):
    def test_case_main(self, case_results):
        ...
        for i in range(0, repeat_times):
            ...
            try:
                ...
                flag1 = settings.set_airplane(time_after_on,time_after_off)
                if not flag1:
                    qsst_log_msg(str(i+1) + '\tairplane mode fail' + '\t' + time.strftime('%X',time.localtime(time.time())))
                    continue
                
                launcher.launch_from_launcher("phone")
                flag2 = phone.phone_call(call_number_1, slot_1, call_duration)
                if not flag2:
                    qsst_log_msg(str(i+1) + '\tvoice call on slot1 fail' + '\t' + time.strftime('%X',time.localtime(time.time())))
                    continue
                
                if SC.PUBLIC_DSDS:
                    launcher.launch_from_launcher("phone")
                    flag3 = phone.phone_call(call_number_2, slot_2, call_duration)
                    if not flag3:
                        qsst_log_msg(str(i+1) + '\tvoice call on slot2 fail' + '\t' + time.strftime('%X',time.localtime(time.time())))
                        continue
