...
class test_suit_cmcc_case5(TestCaseBase):
    TAG = "test_suit_cmcc_case61"
    def test_case_main(self, case_results):

        total_times = 0
        total_success = 0
        repeat_times=int(SC.PUBLIC_CASE_REPEAT_NUMBER)
        address = 'pan.baidu.com/share/link?shareid=298924288&uk=674333259'
        ...
        for i in range(0, repeat_times):
            try:
                ...
                if download_del :
                    ...
                    while search_text('gaodeditu', searchFlag=TEXT_CONTAINS):
                ...
                if flag1 == False:
                    qsst_log_msg(str(i+1) + '\tmms send fail' + '\t' + time.strftime('%X',time.localtime(time.time())))
                    save_fail_log()
...
def download(address):
    ...
    func = lambda:search_text('gaodeditu', searchFlag=TEXT_CONTAINS)
