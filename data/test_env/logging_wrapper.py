...
def qsst_log_reboot_crash(last_log_path, last_suit_name):
    log_path = last_log_path+os.sep+last_suit_name+os.sep+last_suit_name+".qsst"
    line = PREFIX_REBOOT+DIVIDE+REBOOT_CRASH
    ...
    if default_reboot_case_status == FAILED_CASE:
        ...
        __write_log(PREFIX_SEVERITY+DIVIDE+__convert_severity(SEVERITY_HIGH), log_path)
        __write_log(PREFIX_STATUS+DIVIDE+__convert_status(STATUS_UNSUPPORTED)+DIVIDE+"This case crash the phone.", log_path)
...
def do_logcat_capture():
    ...
    if(osInfo == 'Linux-Phone'):
        ...
        cmd = 'adb logcat logcat -d  -v time > ' + logcat_path
            ...
            for line in lines:
                ...
                if index >0 and index < len(line):
                    ...
                    if prefix == PREFIX_LOGGING_PATH:
                        ...
                        if reboot_reason == REBOOT_CRASH:
                            qsst_log_reboot_crash(last_log_path, last_suit_name)
