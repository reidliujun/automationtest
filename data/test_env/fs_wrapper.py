    ...
    if path.exists(search_path):
        if path.isdir(search_path):
            ...
            for tempFileName in curFileNameList:
                if tempFileName.startswith(TEST_SUIT_TAG) and path.isdir(tempFileName):
...
def get_all_cases_py_module_name(suit_name):
    '''
...
def get_test_suit_config(suit_name):
    '''
    ...
    if path.exists(xml_file_name):
        if path.isfile(xml_file_name):
            ...
            for requiredAtrr in SUIT_REQUIRED_ATTRS:
